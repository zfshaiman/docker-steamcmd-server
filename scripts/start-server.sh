...
if [ "${MOD_LAUNCHER}" == "true" ]; then
    cd ${SERVER_DIR}
	screen -S WurmUnlimited -L -Logfile ${SERVER_DIR}/masterLog.0 -d -m ${SERVER_DIR}/WurmServerLauncher-patched SERVERNAME="${WU_SERVERNAME}" SERVERPASSWORD="${WU_PWD}" ADMINPWD="${WU_ADMINPWD}" MAXPLAYERS="${WU_MAXPLAYERS}" EXTERNALPORT="${GAME_PORT}" QUERYPORT="${WU_QUERYPORT}" HOMESERVER="${WU_HOMESERVER}" HOMEKINGDOM="${WU_HOMEKINGDOM}" LOGINSERVER="${WU_LOGINSERVER}" EPICSETTINGS="${WU_EPICSERVERS}" start=${GAME_MODE} ${GAME_PARAMS}
	sleep 2
	tail -f ${SERVER_DIR}/masterLog.0
else
	cd ${SERVER_DIR}
	screen -S WurmUnlimited -L -Logfile ${SERVER_DIR}/masterLog.0 -d -m ${SERVER_DIR}/WurmServerLauncher SERVERNAME="${WU_SERVERNAME}" SERVERPASSWORD="${WU_PWD}" ADMINPWD="${WU_ADMINPWD}" MAXPLAYERS="${WU_MAXPLAYERS}" EXTERNALPORT="${GAME_PORT}" QUERYPORT="${WU_QUERYPORT}" HOMESERVER="${WU_HOMESERVER}" HOMEKINGDOM="${WU_HOMEKINGDOM}" LOGINSERVER="${WU_LOGINSERVER}" EPICSETTINGS="${WU_EPICSERVERS}" start=${GAME_MODE} ${GAME_PARAMS}
	sleep 2
	tail -f ${SERVER_DIR}/masterLog.0
fi